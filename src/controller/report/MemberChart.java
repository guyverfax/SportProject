...
public class MemberChart extends JFrame {
	...
	public MemberChart() {
        ...
        btnBack.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		Tool.goMainUI(MemberChart.this);
        	}
        });
    ...
    private JFreeChart createBarChart() {
        ...
        Map<String, Integer> memberSales = pordersummaryDao.readAmountByMember();

        List<Map.Entry<String, Integer>> sortedEntries = new ArrayList<>(memberSales.entrySet());
        sortedEntries.sort((e1, e2) -> e2.getValue().compareTo(e1.getValue()));
        
        for (Map.Entry<String, Integer> entry : sortedEntries) {
        	Member member=memberDao.readByMemberno(entry.getKey());
        	String memberName=member.getName();
        	dataset.addValue(entry.getValue(), "銷售數量", entry.getKey()+"-"+memberName);
        }
 ...
 // 產生圓餅圖
    private JFreeChart createPieChart() {
        ...
        for (Map.Entry<String, Integer> entry : sortedEntries) {
        	Member member=memberDao.readByMemberno(entry.getKey());
        	String memberName=member.getName();
        	dataset.setValue(entry.getKey()+"-"+memberName, entry.getValue());
        }

        JFreeChart pieChart = ChartFactory.createPieChart(
            "客戶銷售比例", dataset, true, true, false
        );
