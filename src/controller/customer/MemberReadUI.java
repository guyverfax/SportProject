...
public class MemberReadUI extends JFrame {
	...
	public MemberReadUI() {
		...
		btnDetail.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		        ...
		        // 查詢訂單詳情
		        List<Porder> porders = porderDao.readByPorderno(porderNo);
		        new MemberDialog(MemberReadUI.this, porderNo, loginMember, porders ).setVisible(true);
	...
	private void updateTable(List<PorderSummary> orders) {
		DefaultTableModel model = new DefaultTableModel(new String[]{"訂單編號", "訂單日期", "客戶編號", "客戶名字", "員工編號", "員工名字", "產品", "金額"},0)
				{
					@Override
	    ...
	    for (PorderSummary order : orders) {
	        model.addRow(new Object[]{order.getPorderno(), order.getOrderdate(), order.getMemberno(), order.getMembername(), order.getEmployno(), order.getEmployname(), order.getProducts(), order.getTotalprice()});
	    }
	    pordersummaryTable.setModel(model);
		pordersummaryTable.getColumnModel().getColumn(0).setPreferredWidth(110);  // 訂單號碼
		pordersummaryTable.getColumnModel().getColumn(1).setPreferredWidth(50); // 訂單日期
		pordersummaryTable.getColumnModel().getColumn(2).setPreferredWidth(40); // 客戶編號
		pordersummaryTable.getColumnModel().getColumn(3).setPreferredWidth(40); // 客戶名字
		pordersummaryTable.getColumnModel().getColumn(4).setPreferredWidth(40); // 員工編號
		pordersummaryTable.getColumnModel().getColumn(5).setPreferredWidth(40); // 員工名字
		pordersummaryTable.getColumnModel().getColumn(6).setPreferredWidth(200); // 產品
		pordersummaryTable.getColumnModel().getColumn(7).setPreferredWidth(40); // 金額
	}
}
