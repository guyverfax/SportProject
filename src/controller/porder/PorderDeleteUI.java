...
public class PorderDeleteUI extends JFrame {
    ...
    public PorderDeleteUI() {
        ...
        panelTop.setBounds(10, 10, 780, 50);
        ...
        btnDelete.setBounds(118, 399, 100, 30);
        getContentPane().add(btnDelete);

        btnViewDetails = new JButton("查看清單");
        btnViewDetails.setBounds(268, 399, 100, 30);
        ...
        btnBack.setBounds(584, 403, 87, 30);
        getContentPane().add(btnBack);
        
        //infoEmployrole="Admin";//for Testing
        loadPorderData();
    ...
    private void loadPorderData() {
        ...
        if ("Admin".equals(infoEmployrole)) {
            allPorders = pordersummaryDao.readAll();
        } else {
            allPorders = pordersummaryDao.readAllByEmployno(infoEmployno);
        }
    ...
    private void viewPorderDetails() {
        int selectedRow = table.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "請選擇一筆訂單");
            return;
        }
        String porderno = tableModel.getValueAt(selectedRow, 0).toString();
        String products = tableModel.getValueAt(selectedRow, 5).toString();
        String totalPrice = tableModel.getValueAt(selectedRow, 6).toString();
        JOptionPane.showMessageDialog(this, "訂單編號: " + porderno + "\n產品: " + products + "\n總金額: " + totalPrice, "訂單詳情", JOptionPane.INFORMATION_MESSAGE);
    }
